---
title: Business Cycles
draft: false
tags:
  - PECO102
date: 2024-10-15 10:22
---
Discussion [^1]  [^2] [^3] [^4] [^5] [^6] [^7] [^8]
[^9]

# Schools of thought
### Classical
### Jugler
### Schun Peter

### Monetarist
### Keynesian
[^10]

# Kaldor’s Business Cycle

- $I = f(Y,K_-)$ 
	 This happens because if the capital is already high, investments will reduce due to depreciation considerations as the potential returns have reduced.
- $S = f(Y,K_+)$
	 This happens because if we have more capital, economy is booming, more production, less demand, lesser prices, can spend less to consume more and also save more.

![[Business Cycles 2024-10-15 11.07.15.excalidraw.light.svg]]

When MPS > MPI, we have a stable equilibrium and when MPI > MPS, we have an unstable equilibrium. For a stable equilibrium, the investment demand function $I$ should interest the savings supply function $S$ from above thus figure A gives us stable equilibrium and figure B gives us unstable equilibrium.

The limitation of this approach is that both savings and investment functions are linear whereas for business cycles we require non-linearity

# Non-linear Kaldor’s Business Cycle

In non-linear Kaldor’s BC, there are three levels of income - low, normal and high. Both MPI and MPS will not be the same at all three levels.

MPI[^11] will be **relatively low**, at low and high levels of income. This is due to income inelasticity[^12]. 

The MPS will be **relatively high** at low and high levels of income. This is because of income elasticity.

| Income Level | MPI    | MPS    |
| ------------ | ------ | ------ |
| Low          | Low    | High   |
| Normal       | Normal | Normal |
| High         | Low    | High   |


![[Business Cycles 2024-10-15 11.35.41.excalidraw.light.svg]]

Combining the two non-linear investment and savings schedules or functions gives us multiple equilibria, i.e., we have a stable equilibrium at both point $A$ and point $C$, but we have unstable equilibrium at point $B$.

The limitation of this approach is that if you are point $A$, you remain at point $A$. If you are point $C$, you remain at point $C$.

But, if you are at point $B$, you move towards point $A$ or point $B$ depending on which side of $B$ you are on.

![[Business Cycles 2024-10-15 11.51.53.excalidraw.light.svg]]

As the time progresses, the investments and the saving curves, continue on their migrations, induced by capital accumulation, and as the time progresses, the investments and the saving curves, continue on their migrations, induced by capital decumulation.

- Both $Y_A$ and $Y_C$ are short-term equilibrium levels of output.
- The output fluctuation happens between $Y_A$ and $Y_C$.
- 


[^1]: $y*$ is synonymous to multiple things: income, GDP
[^2]: Growth will not remain at 9%, it will change over time
[^3]: Business means your economy and not just that entity that does transactions.
[^4]: Earlier, Juglar talked about these fluctuations and called it “Trade cycles”. It is defined
	- Prosperity (up)
	- Crisis (down)
	- Spread (the time-gap it takes to rise back to prosperity)
[^5]: We will find business cycles mostly into a Market driven economy (private sectors).
	- They are a feature of a capitalist economy
	- In this, they just want to make profits
	- So, they will layoff if it can improve profits (unlike public sector)
	- A private bus service from PTP to Hyderabad was stopped because there were not enough passengers going (not profitable). But the APSRTC still runs with no to two people because it is for the welfare for the people
[^6]: What are the causes of the fluctuations? What to do in order to go back to the equilibrium point.
[^7]: The need for business cycles didn’t arise in the literature of classical models because they always believed in full employment
[^8]: Schun Peter talked about the Innovation theory
	- the business cycle has 4 phases
	- 1. Prosperity, 2. Recession, 3. Depression, 4. Recovery
	- Innovation: finding something new compared to traditional ways, are the cause for the fluctuations
[^9]: Monetarists believed that the fluctuations are because of money supply
[^10]: Even Keynes didn’t talk about business cycles, but what he hinted on was that MEC is the most 
[^11]: Slope of investment demand function
[^12]: non responsive