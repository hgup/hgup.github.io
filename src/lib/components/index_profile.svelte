...
>
  <a class="hidden u-url u-uid" href={site.protocol + site.domain}
    >{site.author.name}</a
  >
  ...
  <div class="text-center flex flex-col gap-2">
    ...
    <p class="opacity-75 p-note whitespace-pre-wrap">{@html site.author.bio}</p>
        ...
        {#each site.author.metadata as { icon, link, rel, text }}
          {#if link}
            ...
            >
              {#if icon}
                {@html icon}
                <!-- <iconify-icon icon={icon} class="!size-5"></iconify-icon> -->
          ...
          {:else}
            <button
              class="btn btn-sm btn-ghost normal-case gap-2"
              class:btn-square={!text}
            >
              {#if icon}
                <span class="{icon} !w-5 !h-5">{icon}</span>
